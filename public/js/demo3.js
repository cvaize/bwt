...
window.jquery = window.$;
var width = $('.hover-blob').width();
var count = Math.round(width / 50 + 4);
...
var active = false;
var vi = 1;
var blobFun = function blobFun() {
    var firstItem = $(hoverItems[0]);
    var firstItemX = firstItem.offset().left + 25 - cursore.pageX;
    var firstItemY = firstItem.offset().top + 25 - cursore.pageY;
    var rand = Math.sin(vi);
    if (rand < 0.5) {
        rand = 1;
    }
...
$('.header__item').hover(function () {
    var blobs = $(this).find('.blob');
    var colorT = $(blobs[0]).css('background-color');
